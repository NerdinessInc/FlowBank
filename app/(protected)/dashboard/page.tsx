...
// services
import { ReturnAcctDetails2, getAccessCode } from '@/services/api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (userData?.acctCollection?.AcctStruct) {
      ReturnAcctDetails2(2, userData.userRec, userData.acctCollection.AcctStruct)
        .then((res: any) => {
          if (res.success) {
  ...
  const selectAccountByNumber = (accountNumber: string) => {
    const selected = accountDetails.find((account: { accountNumber: string }) => account.accountNumber === accountNumber);
